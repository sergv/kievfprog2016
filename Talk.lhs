...
\title{Let’s play with Regular Expressions}
...
\begin{frame}[fragile]{Introduction}
  ...
  \begin{itemize}[<+->]
    \item This talk is based on “A Play on Regular Expressions” Functional Pearl by Sebastian Fischer, Frank Huch and Thomas Wilke
    \item Regular expressions are usually taken for granted and are implemented in C
  \end{itemize}
...
\begin{frame}[fragile]{Why reimplement regular expressions in Haskell}

  \begin{itemize}[<+->]
    \item Some widespread implementations, e.g. Perl, Python, Java take exponential time on some inputs - we can do better
      \begin{itemize}
        \item Not all implementations - grep and tcl, among others, are OK
      \end{itemize}
    \item Foreign libraries are harder to integrate with pure code
      \begin{itemize}
        \item Have a trie data structure, want all entries where prefix matches a regex. Can do this naively but will have to rerun maching process for each prefix
        \item Compiling via GHCJS and can’t link to C libraries
      \end{itemize}
    \item Can do symbolic manipulation on regular expression AST - just for fun
  \end{itemize}
